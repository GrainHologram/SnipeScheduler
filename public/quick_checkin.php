    ...
    if ($mode === 'add_asset') {
        ...
        if (empty($checkinAssets)) {
            ...
            if (empty($errors)) {
                ...
                // Transition checked_out reservations to completed when all assets returned
                $checkedInTagsByUser = [];
                $checkedInAssetIds   = [];
                foreach ($checkinAssets as $asset) {
                    if (empty($asset['checked_out'])) {
                        continue;
                    }
                    $uid = (int)($asset['assigned_id'] ?? 0);
                    if ($uid <= 0) {
                        continue;
                    }
                    $checkedInTagsByUser[$uid][] = $asset['asset_tag'] ?? '';
                    $checkedInAssetIds[] = (int)$asset['id'];
                }

                foreach ($checkedInTagsByUser as $uid => $tags) {
                    // Find checked_out reservations for this user
                    $resStmt = $pdo->prepare("
                    ...
                    // Get remaining checked-out assets for this user (excluding ones just checked in)
                    $remainingTags = [];
                    $placeholders = implode(',', array_fill(0, count($checkedInAssetIds), '?'));
                    $remStmt = $pdo->prepare("
                        SELECT asset_tag
                    ...
                    ");
                    $remParams = array_merge([$uid], $checkedInAssetIds);
                    $remStmt->execute($remParams);
                    while ($row = $remStmt->fetch(PDO::FETCH_ASSOC)) {
                        $remainingTags[] = $row['asset_tag'];
                    }
                    ...
                    foreach ($userReservations as $res) {
                        ...
                        // Check if any of the reservation's asset tags are still checked out
                        $stillOut = false;
                        foreach ($resTags as $resTag) {
                            if ($resTag !== '' && in_array($resTag, $remainingTags, true)) {
                                $stillOut = true;
                                break;
                            }
                        }
                        ...
                        if (!$stillOut) {
                            ...
                            activity_log_event('reservation_completed', 'Reservation completed (all assets returned)', [
                                ...
                                'metadata'     => [
                                    'completed_via' => 'quick_checkin',
                                ],
                            ]);
                            $messages[] = "Reservation #{$res['id']} marked as completed (all assets returned).";
                        }
                    }
                }
