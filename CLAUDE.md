...
### Directory Layout
- **`public/`** — Web root (Apache/Nginx should point here). Contains all user-facing PHP pages, the installer, and static assets (`assets/style.css`, `assets/style-v2.css`, `assets/nav.js`). `nav.js` contains three v2-specific JS modules in addition to the nav init code: `TopbarCrumbs` (breadcrumb stack API), `CatalogueDetail` (inline item detail view), and the global `openModelDetail()` entry point.
...
v2 uses a fixed left sidebar for navigation and a fixed top bar for page context. Key CSS variables: `--sidebar-width`, `--topbar-height`, `--mobile-nav-height`, `--catalogue-sidebar-width` (all defined in `:root` of `style-v2.css`).

`.catalogue-main` is `position: fixed` — this is intentional so its flex children can scroll independently. `body.page-catalogue { overflow: hidden }` prevents body scrolling at desktop/tablet. On mobile (≤599px) `.catalogue-main` reverts to `position: static`.
...
#### Design v2 Navigation Structure

The v2 sidebar nav contains a brand link, section headers, nav links, a feedback glyph button (`.app-nav-feedback-glyph`, `bi-chat-left-dots`, opens feedback modal, sits above user card), and a user card at the bottom (`.app-nav-user`, `id="accountPanelTrigger"`, opens account panel slide-in).

After `</nav>`, `layout_render_nav()` outputs (gated `$designVersion >= 2`): an account panel backdrop (`#accountPanelBackdrop`) and account panel (`#accountPanel`, slide-in from left, AJAX-loads `ajax_account_panel.php`).

`TopbarCrumbs.push/pop/popAll()` in `nav.js` manages breadcrumb depth beyond the server-rendered subtitle. Only call in a v2 context. The subtitle span has `id="app-topbar-crumb-1"` — stable JS anchor.

#### Elements hidden in v2 (`style-v2.css`)

- `.app-logo`, `.page-header`, `.page-subtitle`, `.top-bar`, `.catalogue-tab-nav`, `#catalogue-window-form`

#### Elements hidden in v1 (`style.css`)

- `.app-topbar`, `.app-nav-brand`, `.app-nav-user`, `.app-nav-header`, `.app-nav-feedback-glyph`
- `.my-bookings-panels`, `.my-bookings-panel`, `.my-bookings-panel-title`, `.my-bookings-panel-box`
- `.panel-empty-state`, `.panel-empty-icon`, `.panel-empty-text`
- `.cat-sidebar-user-card`, `.cat-sidebar-window-display`, `.cat-sidebar-section-divider`
- `#windowModalBackdrop`, `#windowModal`

#### Colour System (`style-v2.css`)

All colours are CSS custom properties in the `:root` block of `style-v2.css`.
...
#### Pages with `$designVersion` branching — non-obvious gotchas only

**`public/catalogue.php`:**
- Detects `$designVersion` from already-loaded `$config` (not `load_config()`).
...
- `layout_model_history_modal()` is only emitted for v1 (`$designVersion < 2`); v2 uses `<script>window._modelDetailIsStaff = ...</script>` instead.

**`public/my_bookings.php`:**
- Detects `$designVersion` via `load_config()` (not `$config` — different from catalogue.php).
